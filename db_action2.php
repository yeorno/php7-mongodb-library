...
class Action
{
  ...
  function __construct($pre,$dbuser,$dbpass,$host,$using_db)
  {
    $this->pre=$pre;
    $this->dbsuer=$dbuser;
    $this->dbpass=$dbpass;
    $this->host=$host;
    $this->using_db=$using_db;
  ...
  public function add($db,$data=array()){
    ...
    $using_db=$this->using_db;
    $bulk = new MongoDB\Driver\BulkWrite;
    $_id=$bulk->insert($data);
    
    $list=$manager->executeBulkWrite($using_db.'.'.$db, $bulk);
  ...
  public function updates($db,$where=array(),$set=array(),$des=array('multi' =>false, 'upsert' =>false)){
    ...
    $result=$manager->executeBulkWrite($using_db.'.'.$db, $bulk, $writeConcern);
  ...
  public function select($db,$where=array(),$option=array()){
    ...
    $cursor = $manager->executeQuery($using_db.'.'.$db,$query);
 ...
 /*distinct method*/
  public function distinct($db,$key,$where=''){
    ...
    $cursor = $manager->executeCommand($using_db, $cmd); // retrieve the results
   ...
   public function rand_($db,$where='',$count=1){//默认随机返回一条数据
      ...
      $cursor = $manager->executeCommand($using_db, $command);
      foreach ($cursor as $key => $value) {
        $list[]=json_decode(json_encode($value),true);
      }
      if (count($list)==1) {
